...
class CasualSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # calculate query, key and value for al the heads in batch and move head forward to be the batch
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        config_args['vocab_size'] = 50257 # always 50257 from GPT model checkpoints
        ...
        # basically the openai checkpoints use a "Conv1D", but we only want to use a vanilla layer
...
while x.size(1) < max_length:
    ...
    with torch.no_grad():
        ...
        # topk_probs here becomes (5, 50), topk_indeices is (5, 50)
        topk_probs, topk_indices = torch.topk(probs, 50, dim = -1)
        # select a token from the top-k probabiliites
